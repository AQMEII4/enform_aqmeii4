...
C
      SUBROUTINE PARSE_METAFILE_AQR_HIRES(LUN)
C
...
C --- CALLS        : readrec.for
C
C --- VERSION      : 20150420 - RBI - Forked from parse_metafile_aqr.for
C                  : 20150928 - RBI - Fix to DELTA_CC calculation
C                                   - Fix to UTM receptors read
C
C--------------------------------------------PARSE_METAFILE_AQR_HIRES---
C
      IMPLICIT NONE
...
C
      ...
      CHARACTER     HEM*1
      CHARACTER     TMP_DATE*12, TMP_DATE_HIRES*16
      REAL          ARRAY1( MAXVAL )
...
C --- First output (UTC) 
      ...
      CALL READREC( DUMMY_READ, NL, IVAR, NDIMR, ARRAY1, ARRAY2,
     &              ARRAY3, ARRAY4, ARRAY5 )
      TMP_DATE_HIRES = ARRAY5
      READ(TMP_DATE_HIRES,'(I4.4,6I2.2)') SOURCE_FIRST_OUTPUT
...
C --- Time horizon (UTC)
     ...
     &              ARRAY3, ARRAY4, ARRAY5 )
      TMP_DATE_HIRES = ARRAY5
      READ(TMP_DATE_HIRES,'(I4.4,6I2.2)') TIME_HORIZON
...
C     ending averaging time within output period. [in seconds]
      READ(LUN,'(A)') DUMMY_READ
      IF (LDEBUG) THEN
          WRITE(6,*) DUMMY_READ
      ENDIF
      ...
      CALL READREC( DUMMY_READ, NL, IVAR, NDIMR, ARRAY1, ARRAY2,
     &              ARRAY3, ARRAY4, ARRAY5 )
      !DELTA_CC = ARRAY1(1) out 20150930
      DELTA_CC = NINT(ARRAY1(1) * 100)
      DTOS = ARRAY1(2)
      DTOE = ARRAY1(3)
...
C --- Coordinates of discrete receptors
      DO IDR = 1, SOURCE_NDISCREC
        IF (COORDINATES(1:LSTRIM(COORDINATES)) .EQ. 'UTM') THEN
          ...
          ! SOURCE_RECEPTOR_UTMZONE(IDR) = UTMZONE out 20150928
          ! SOURCE_RECEPTOR_HEMISPHERE(IDR) = HEMISPHER out 20150928E
        ELSE
          READ(LUN,*) IR, RX, RY
...
950   WRITE (6,*) 'ERROR (PARSE_METAFILE_AQR_HIRES)'
